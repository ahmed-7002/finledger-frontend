...
export default function EditCustomerModal({ customer, tenant, onClose }) {
  const [name, setName] = useState(customer.name);
  const [phone, setPhone] = useState(customer.phone);
  const [nationalId, setNationalId] = useState(customer.national_id || "");
  const [phoneVerified, setPhoneVerified] = useState(Boolean(customer.phone_verified));
  ...
  function handlePhoneChange(value) {
    setPhone(value);
    // Editing the number invalidates any prior verification - it was only
    // ever proof for the old number, not this new one.
    if (value !== customer.phone) {
      setPhoneVerified(false);
    } else {
      setPhoneVerified(Boolean(customer.phone_verified));
    }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4">
      <div className="bg-surface-container-lowest w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-headline text-xl font-semibold text-primary">Edit Profile</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-primary/70 mb-1">National ID</label>
            <input
              value={nationalId}
          ...
          <button
            ...
            className="w-full bg-primary text-on-primary rounded-xl py-3 font-medium text-sm disabled:opacity-60"
          >
